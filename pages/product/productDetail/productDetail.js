...
Page({
  ...
  getPrice: function() {
    ...
    http.$request(api.GoodsParams, {
      goodsId: id,
      standard: JSON.stringify(dataObj)
    }).then(function(res) {
      wx.hideLoading();
      const data = res.data;
      let attachs = [];
  ...
  bookPurchase: function() {
    this.BookOrPhurse(2);
  },
  BookOrPhurse: function(type) {
    var _this = this;
    if (this.data.openAttr == true) {
      ...
      if(type == '1'){
        ...
        let speId = this.data.speId;
        console.log(speId+"__________")
        _this.closeBox();
        wx.navigateTo({
          url: '/pages/shopCart/applyOrders/applyOrders?speId=' + speId,
        })
      }
